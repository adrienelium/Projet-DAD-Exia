...
public class ConnectDB {
    ...
    public Boolean GetListeMot(String txt){
        ...
        */ 
        ...
        List<Mot> liste_mot= (List<Mot>)this.em.createNamedQuery("Mot.findByMot");
        //List<Mot> liste_mot= (List<Mot>)this.em.createNamedQuery("Mot.findByMot").setParameter("mot", txt).getResultList();
        //List<Mot> liste_mot= this.em.createQuery("SELECT m FROM Mot m").getResultList();
        ...
        if(!liste_mot.isEmpty()){
                System.out.println("Le mot à été trouvé dans la base de données : " + liste_mot.get(0).getMot().toString());
                return true;
        }
